        ...
        if(permission('category-access')){
            if($request->ajax()){
                ...
                foreach ($list as $value) {
                    ...
                    $row[] = $no;
                    $row[] = $value->name;
                    $row[] = table_image($value->image,CATEGORY_IMAGE_PATH,$value->name);
                    $row[] = $value->category_text;
                    $row[] = permission('category-edit') ? change_status($value->id,$value->status,$value->name) : STATUS_LABEL[$value->status];;
        ...
        if($request->ajax()){
            if(permission('category-add') || permission('category-edit')){
                ...
                if($request->hasFile('image')){
                    $image = $this->upload_file($request->file('image'),CATEGORY_IMAGE_PATH);

                    if(!empty($request->old_image)){
                        $this->delete_file($request->old_image,CATEGORY_IMAGE_PATH);
                    }
                }

                $lifestyle_image = $request->old_lifestyle_image;
                if($request->hasFile('lifestyle_image')){
                    $lifestyle_image = $this->upload_file($request->file('lifestyle_image'),CATEGORY_IMAGE_PATH);

                    if(!empty($request->old_lifestyle_image)){
                        $this->delete_file($request->old_lifestyle_image,CATEGORY_IMAGE_PATH);
                    }
                }

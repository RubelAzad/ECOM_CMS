    ...
    <div class="row">
        ...
        <div class="col-xl-12">
            ...
            <div class="dt-entry__header">
                ...
                <!-- /entry heading -->
                @if (permission('cmodule-add'))
                <button class="btn btn-primary btn-sm" onclick="showFormModal('Add New Content Module','Save');clearOldImage()">
                    ...
                    <form id="form-filter">
                        <div class="row">
                            <div class="form-group col-md-4">
                                <label for="name">Content Module Name</label>
                                <input type="text" class="form-control" name="name" id="name" placeholder="Enter Content Module Name">
                            </div>
...
$(document).ready(function(){
    ...
    table = $('#dataTable').DataTable({
        ...
        "ajax": {
            "url": "{{route('cmodule.datatable.data')}}",
            "type": "POST",
            "data": function (data) {
    ...
    $(document).on('click', '#save-btn', function () {
        ...
        let url = "{{route('cmodule.store.or.update')}}";
        ...
        if (id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#store_or_update_form #module_description').val(data.module_description);
                    $('#store_or_update_form #module_color').val(data.module_color);
                    $('#store_or_update_form #old_image').val(data.image);
                    $('#store_or_update_form .selectpicker').selectpicker('refresh');
                    if(data.image){
                        var image = "{{ asset('storage/'.CONTENT_MODULE_IMAGE_PATH)}}/"+data.image;
    ...
    function multi_delete(){
        ...
        if(ids.length == 0){
            ...
            let url = "{{route('cmodule.bulk.delete')}}";
    ...
    $(document).on('click', '.change_status', function () {
        ...
        let name  = $(this).data('name');
        let row   = table.row($(this).parent('tr'));
        let url   = "{{ route('cmodule.change.status') }}";
        change_status(id,status,name,table,url);

    });

    $(document).on('click', '.module_field_change_status', function () {
        let id    = $(this).data('id');
        let status = $(this).data('status');
        let name  = $(this).data('name');
        let row   = table.row($(this).parent('tr'));
        let url   = "{{ route('cmodule.module.field.change.status') }}";
        module_field_change_status(id,status,name,table,url);

    ...
    <div class="row">
        ...
        <div class="col-xl-12">
            ...
            <div class="dt-entry__header">
                ...
                <!-- /entry heading -->
                @if (permission('combocategory-add'))
                <button class="btn btn-primary btn-sm" onclick="showFormModal('Add New ComboCategory','Save');clearOldImage()">
                    ...
                    <form id="form-filter">
                        <div class="row">
                            <div class="form-group col-md-4">
                                <label for="name">ComboCategory Name</label>
                                <input type="text" class="form-control" name="name" id="name" placeholder="Enter ComboCategory Name">
                            </div>
...
$(document).ready(function(){
    ...
    table = $('#dataTable').DataTable({
        ...
        "ajax": {
            "url": "{{route('combocategory.datatable.data')}}",
            "type": "POST",
            "data": function (data) {
        ...
        "buttons": [
            @if (permission('combocategory-report'))
            {
    ...
    $(document).on('click', '#save-btn', function () {
        ...
        let url = "{{route('combocategory.store.or.update')}}";
        ...
        if (id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data.image){
                        var image = "{{ asset('storage/'.ComboCategory_IMAGE_PATH)}}/"+data.image;
    ...
    $(document).on('click', '.delete_data', function () {
        ...
        let url   = "{{ route('combocategory.delete') }}";
    ...
    function multi_delete(){
        ...
        if(ids.length == 0){
            ...
            let url = "{{route('combocategory.bulk.delete')}}";
    ...
    $(document).on('click', '.change_status', function () {
        ...
        let name  = $(this).data('name');
        let row   = table.row($(this).parent('tr'));
        let url   = "{{ route('combocategory.change.status') }}";

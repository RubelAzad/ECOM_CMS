...
<table style="width: 100%; max-width: 600px; margin: 0 auto; background-color: #ffffff; padding: 20px;">
    ...
    <tr>
        <td style="padding: 20px;">
            <p>Dear {{$orderStatus['customer_name']}},</p>
            <p>Thank you for your order! We're excited to let you know that your order has been confirmed and is being
                processed.</p>
            <h2>Order Details:</h2>
            <h4>Order ID: {{$orderStatus['order_id'] }}</h4>
            <h4>Order Status: {{ $orderStatus['status'] }}</h4>
            <h4>Payment status: {{ $orderStatus['payment_status'] }}</h4>
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                @foreach($orderStatus['orderItems'] as $orderItem)
                    <tr>
            ...
            <p style="text-align: right; margin-top: 10px; margin-right:10px;">Sub Total: {{ $orderStatus['sub_total'] }} Tk</p>
            <p style="text-align: right; margin-top: 10px; margin-right:10px;">Shipping Charge: {{ $orderStatus['shipping_charge'] }} Tk</p>
            <p style="text-align: right; margin-top: 10px; margin-right:10px;"><strong>Total: {{ $orderStatus['grand_total'] }} Tk</strong></p>
